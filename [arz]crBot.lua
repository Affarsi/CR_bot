...
function main()
    ...
    asyncHttpRequest('GET',
        ...
        for k,v in pairs(f()) do
            if v.version > tonumber(thisScript().version) then
                sampAddChatMessage('{F8B32D}Вышло обновление на версию %s, подробности в /crb',v.version)
                update = v
                break
            end
        end
    ...
    while true do wait(0)
        imgui.Process = window.v or lavki.window.v or bot.viewItems.v
...
function imgui.OnDrawFrame()
    ...
    if bot.viewItems.v then
        imgui.SetNextWindowSize(imgui.ImVec2(350,350),2)
        imgui.SetNextWindowPos(imgui.ImVec2(sw/1.3-350/2,sh/2-350/2),2)
        imgui.Begin(u8'Предметы для скупки',bot.viewItems)
        for k,v in ipairs(j.bot.items) do
            imgui.Text(u8(string.format('%s\t%s',v.item,(v.buy and string.format('%s кол-во по %s$',v.int,v.cost) or ' чекнуть'))))
        end
        imgui.End()
    end
   ...
   if window.v then
        ...
        if active == b[1] then
            ...
            if imgui.Button('github repository script') then
                os.execute('explorer "https://github.com/v3sp4n/crBot"')
            end
            imgui.PopFont()
...
Когда бот начал/сделал круг
        ...
        elseif active == b[3] then
            ...
            if not bot.listItems then
                if imgui.DragInt('delay',bot.delay,0.005,1,1000) then;   j.bot.delay = bot.delay.v;  json.save(j,'[arz]crBot.json');   end
...
(если поблизости Свободная лавка то скрипт её пропустит)]])
                ...
                if imgui.CollapsingHeader(u8'\tЧто я скупаю?') then
                    ...
                    for k,v in ipairs(j.bot.items) do
                        ...
                        imgui.NextColumn()
                        local int = imgui.ImInt(v.int)
                        local cost = imgui.ImInt(v.cost)
...
(сайт возможно не будет работать в Украине(но у меня работал, тут зависит от вашего провайдера))
                    ...
                    if imgui.Button(u8'Установить json из github репозиторий скрипта') then
                        asyncHttpRequest('GET',
                            'https://raw.githubusercontent.com/v3sp4n/crBot/main/%5Barz%5DCrBaron_Items.json',nil,function(r)
...
Ну, или вручную добавить товар в json(moonloader/config/[arz]crBot_Items.json)..]])
    ...
    то если лавка слетит и скрипт успешно её купит все другие лавки отключат ловлю]])
            ...
            imgui.SetCursorPos(imgui.ImVec2(0,170))
            for k,v in ipairs(b) do
                if k == 3 then
                    imgui.Spacing()
                end
